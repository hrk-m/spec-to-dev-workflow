...
グループ詳細画面の Subgroups セクションヘッダーにある「追加」ボタンをクリックすると、AddSubgroupSheet（シート）が表示される。シート内でグループを検索・ラジオ選択し、「追加」ボタンで選択したグループをサブグループ（子グループ）として登録できる。すでに直接の子グループになっているグループは候補から除外される。
...
- [ ] サブグループが 1 件以上のとき Subgroups セクションにグループ名・説明・メンバー数（"N members"）が表示される
