...
グループ詳細が表示される / メンバー検索キーワードを入力する
  ...
  ├─ メンバーが uuid・姓名の 2 列テーブル形式で表示される（削除モード時はチェックボックス列が追加される）
...
| `MemberRow`      | コンポーネント | 個々のメンバー行（uuid・姓名の 2 列）を表示する。`data-testid="member-row"` が付与されており E2E テストのセレクターとして使用される |
