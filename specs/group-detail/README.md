...
| メンバー総件数バッジ               | サーバー集計の重複除外済み件数（`apiTotal`）を「N件」で表示する。フィルター変化中は `member_count + 選択中サブグループのメンバー数合計` をフォールバック表示する                                                            |
| 重複件数バッジ                     | 複数サブグループに所属する実ユーザー数を「重複 N件」で表示する（BE の `duplicate_count`）。0 のとき非表示                                                                                                                   |
...
| メンバー総件数バッジ       | 重複除外済み件数（`apiTotal`）または `member_count + 選択中サブグループのメンバー数合計` をフォールバック表示        |
| 重複件数バッジ             | `duplicate_count` を「重複 N件」で表示する。0 のとき非表示                                                           |
