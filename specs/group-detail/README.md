...
特定のグループの詳細情報とそのメンバー一覧・サブグループ一覧を確認する画面。グループの名称・説明を表示し、所属メンバーを検索・無限スクロール付きで閲覧できる。また、Subgroups セクションからサブグループの追加・閲覧ができる。
...
10. > ボタンを押す / ESC キーを押す / シート外（左端エリア）をクリックする → シートが右へスライドアウトして閉じる
11. ヘッダーの ↔ ボタンを押す → シートが消えてフルページ（`/groups/:id`）に展開される。ブラウザ戻るで一覧に戻る
...
9. 「< Groups」ボタンを押す → グループ一覧画面（`/`）へ遷移する
