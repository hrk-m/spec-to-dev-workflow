...
| メンバーリスト（クリック可能） | イニシャルアバターとフルネームでメンバーを一覧表示する。行クリックで MemberDetailSheet が開く |
...
9. メンバー行のチェックボックスをクリックして 1 件以上選択する → 「削除」ボタンが enabled になる
...
5. 「Next」ボタンを押す → 次のページのメンバーが表示される
6. 「Previous」ボタンを押す → 前のページのメンバーが表示される
7. 「メンバー追加」ボタンをクリックする → AddMemberSheet が表示され、未所属ユーザー一覧が描画される
8. メンバー行のチェックボックスをクリックして 1 件以上選択する → 「削除」ボタンが enabled になる
9. 「削除」ボタンをクリックする → 削除確認 AlertDialog が表示される（「選択した N 名をグループから削除しますか？」）
10. ダイアログの「削除する」をクリックする → メンバーが削除され、メンバー数と一覧が更新される
