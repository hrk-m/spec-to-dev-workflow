...
画面が表示される
  ...
  ├─ バックエンドからユーザー一覧（id, uuid, first_name, last_name）と total が返る
  ├─ cachedUsers にキャッシュされ、テーブルに id / uuid / 姓名 が表示される
...
```
- [ ] 画面を開くとスケルトン表示後にユーザー一覧テーブルが表示される
- [ ] テーブルヘッダーに id / uuid / 姓名 の列ヘッダーが存在する
- [ ] テーブルの各行に id・uuid・姓名が正しく表示される
- [ ] uuid 列に UUID 形式（xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx）の文字列が表示される
- [ ] アバターアイコンが表示されない
- [ ] 初回マウント時に GET /api/v1/users?limit=100&offset=0 が呼び出される
- [ ] ヘッダーサブタイトルにユーザー総件数（total）が表示される
- [ ] 検索ボックスにキーワードを入力すると 300ms 後に一覧が絞り込まれる
- [ ] 検索をクリアすると全件が再表示される
- [ ] スクロールしてテーブル末尾に到達すると次バッチが自動でロードされる（無限スクロール）
