...
| 目的         | グループ詳細への遷移をページ全体の切り替えからスライドインシートに変更し、コンテキストを保ちながら詳細・ネスト画面を表示できるようにする |
...
| 3 | 各シートの × ボタンクリック | 最前面シートを 1 段閉じる（右へスライドアウト） |
...
```
1. ユーザーが × ボタン or 左端 10% エリア or ESC キーを操作
2. closeSheet() を呼ぶ
...
| 2 | Sheet | × ボタンクリックで onClose が呼ばれる | onClose 1 回呼び出し |
| 3 | Sheet | ESC キーで onClose が呼ばれる | onClose 1 回呼び出し |
| 4 | SheetStackContext | openSheet でスタックに 1 枚追加される | sheets.length が増える |
| 5 | SheetStackContext | closeSheet でスタックから 1 枚削除される | sheets.length が減る |
| 6 | GroupList | 行クリックで openSheet が呼ばれる（navigate は呼ばれない） | openSheet の呼び出し確認 |
| 7 | MemberList | onMemberClick が渡されたときメンバー行クリックで呼ばれる | onMemberClick の呼び出し確認 |
| 8 | MemberDetailSheet | member props のメンバー名が表示される | 姓名のレンダリング確認 |
| 9 | 既存テスト | GroupDetailPage.test / MemberList.test が壊れない | 全 pass 維持 |
...
| ファイル | 変更内容 |
| -------- | -------- |
| `sample-front/src/app/router.tsx` | Layout + HomePageWithSheets を追加（SheetStackProvider・IoC wiring） |
...
| ファイル | 理由 |
| -------- | ---- |
| `sample-front/src/pages/group-detail/ui/GroupDetailPage.tsx` | /groups/:id 直接アクセス用のフルページ表示を維持 |
...
7. × ボタン・左端 10% エリアクリック・ESC キーで最前面シートを 1 段閉じる
...
11. `make test`・`make lint`・`make build` がすべて pass する
