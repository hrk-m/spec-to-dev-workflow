...
| 対応ステップ  | パス                                            | 役割                                                                                 |
| ------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------ |
| 5-2           | `sample-api/domain/user.go`                     | User Entity（UUID string フィールドを含む）                                          |
| 5-2           | `sample-api/group/service.go`                   | GroupRepository interface・ビジネスロジック                                          |
| 5-5           | `sample-api/group/service_test.go`              | Service ユニットテスト（mock データの `domain.User` に UUID を含む）                 |
| 5-1, 5-2, 5-4 | `sample-api/internal/rest/group.go`             | HTTP Handler（ListNonGroupMembers）                                                  |
| 5-5           | `sample-api/internal/rest/group_test.go`        | Handler ユニットテスト（mock データに UUID を含む・`uuid` フィールドのアサーション） |
| 5-2, 5-3      | `sample-api/internal/repository/mysql/group.go` | ListNonGroupMembers（SELECT に `uuid` を含む・Scan で `&u.UUID` を取得）             |

### sample-front

| 対応ステップ | パス                                                                       | 役割                                                                                                                                                  |
| ------------ | -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| 5-2-FE       | `sample-front/src/pages/group-detail/ui/AddMemberSheet.tsx`                | 列構成 □選択 / uuid / 姓名 のテーブル表示。スケルトン・空データ行の `colSpan=3`。データセルに `user.uuid` を表示し、選択・削除キーは `user.id` を使用 |
| 5-2-FE       | `sample-front/src/pages/group-detail/ui/AddMemberSheet.styles.ts`          | `tableCellUuid` スタイル（MemberList の `tableCellId` と同パターン: 幅固定・`textSecondary` カラー）                                                  |
| 5-5          | `sample-front/src/pages/group-detail/ui/__tests__/AddMemberSheet.test.tsx` | AddMemberSheet テスト（uuid 列ヘッダー確認・モックデータ `GroupMember` は `uuid` フィールドを持つ）                                                   |
| 5-2-FE       | `sample-front/src/pages/group-detail/api/fetch-non-members.ts`             | `GET /api/v1/groups/:id/non-members` 呼び出し（`GroupMember` 型は `entities/group/model/group-member.ts` で管理）                                     |
...
| 23  | モックデータ | テスト内の `GroupMember` モックオブジェクトに `uuid` が含まれる | TypeScript コンパイルエラーなし                      |
...
7. AddMemberSheet のユーザーリストが `<table>` 形式（`<table>` + `<thead>` + `<tbody>`）で表示される
8. `<thead>` に空の `<th>`（チェックボックス列、`aria-label="選択"`）・`<th>uuid</th>`・`<th>姓名</th>` の 3 列ヘッダーが存在する
9. アバターアイコン（イニシャル円形）を表示しない（各ユーザー行に `UserAvatar` は使用しない）
10. `columnheader` ロールで `uuid` および `姓名` が取得できる
11. `AddMemberSheet.styles.ts` に `tableCellUuid` スタイルが定義されている（MemberList の `tableCellId` と同パターン）
