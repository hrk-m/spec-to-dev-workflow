...
- 戻り値型は `domain.GroupMember`（`ID uint64` + `UUID string` + `FirstName string` + `LastName string` + `SearchKey string` + `Sources []GroupMemberSource`）を `domain/group.go` に定義する
- `domain.GroupMemberSource` は `GroupID uint64` + `GroupName string` のペアで複数所属元を表す
...
| 対応ステップ | パス                                                   | 役割                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------------ | ------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 5-2          | `sample-api/domain/group.go`                           | `GroupMember`（`ID`, `UUID`, `FirstName`, `LastName`, `SearchKey`, `Sources []GroupMemberSource`）と `GroupMemberSource`（`GroupID uint64`, `GroupName string`）を定義                                                                                                                                                                                                                                |
...
| 5-2, 5-4     | `sample-api/internal/rest/group.go`                    | `GroupService.ListGroupMembers` IF の戻り値型を `([]domain.GroupMember, int, error)` に変更（消費側 IF 宣言を更新）・`groupMemberItem` の `Sources []groupMemberSourceItem` を JSON `source_groups` として返す・`getStatusCode` の `ErrBadParamInput` / `ErrNotFound` / `ErrInternalServerError` マッピングが既に存在することを確認                                                                   |
  ...
  "members": [
    {
      ...
      "last_name": "山田",
      "source_groups": [
        { "group_id": 10, "group_name": "Engineering" }
      ]
    },
    {
      "id": 5,
      ...
      "last_name": "佐藤",
      "source_groups": [
        { "group_id": 11, "group_name": "Frontend Team" }
      ]
    }
  ],
  "total": 250
...
17. 戻り値型は `domain.GroupMember`（既存 `domain.User` 全フィールド + `Sources []GroupMemberSource`）であり、`domain/group.go` に新設する。既存 `domain.User` は変更しない
