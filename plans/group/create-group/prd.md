...
```
1. リクエストボディを c.Bind(&req) でパース（失敗時 400）
...
| `src/pages/home/model/__tests__/useCreateGroup.test.ts` | 新規: フックテスト（model セグメントのコードは model/__tests__/ に配置） |
