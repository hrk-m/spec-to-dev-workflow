...
1. `/users` でユーザー一覧画面（UserList コンポーネント）が表示される
2. マウント時に `GET /api/v1/users?limit=100&offset=0` を呼び出し、最大 100 件を取得してクライアントキャッシュする
3. 取得した全件をテーブル形式（`<table>` ベース）で即時表示する。テーブルヘッダー行に id・uuid・姓名 を表示する
4. 各ユーザー行（`<tr>`）に id・uuid・姓名（last_name + first_name 順）を表示する
5. アバターアイコン（姓名の頭文字円形アイコン）を表示しない
6. センチネル要素（IntersectionObserver 監視）が viewport に入り、かつ直前バッチサイズが `FETCH_LIMIT`（100）と等しい場合に追加フェッチする
7. 追加フェッチ中はテーブル末尾にスピナーを表示する
8. 追加フェッチ失敗時はテーブル末尾にエラーメッセージを表示する（既存表示アイテムは維持）
9. キーワード入力後 300ms デバウンスで `q={keyword}` を付けて再取得する（ユーザー一覧リセット・`offset=0` リセットあり）
10. データ取得中はスケルトンローディングをテーブル行形式（3 列）で表示する
11. 検索 0 件時は "No users found" を userCountLabel（ページサブタイトル）に表示する
12. エラー時は "Couldn't load users" エラーカードを表示する
13. `User` 型に `id`・`uuid`・`first_name`・`last_name` フィールドが含まれ、全フロントエンドテストが通過する
