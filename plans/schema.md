...
| `group_members` | グループに所属するメンバーを表す関連テーブル | グループ集約の子エンティティ |
...
**概要**: グループに所属するメンバーを表す。`group_id` で `groups` と紐付く。
...
| カラム名 | データ型 | NULL | デフォルト | 説明 |
|---|---|---|---|---|
| `id` | `BIGINT UNSIGNED` | NOT NULL | AUTO_INCREMENT | メンバーの識別子（主キー） |
| `group_id` | `BIGINT UNSIGNED` | NOT NULL | なし | 所属するグループの ID（FK → `groups.id`） |
| `name` | `VARCHAR(255)` | NOT NULL | なし | メンバー名 |

#### 制約

| 種別 | 名前 | 対象カラム | 説明 |
|---|---|---|---|
| PRIMARY KEY | `PRIMARY` | `id` | メンバーの一意識別子 |
...
|---|---|---|
| `groups` | API 経由（ユーザー操作） / シードデータ（`002_seed_groups.sql`） | `GET /api/v1/groups` — グループ一覧取得 |
| `group_members` | API 経由（ユーザー操作） / シードデータ（`002_seed_groups.sql`） | `GET /api/v1/groups` — グループごとのメンバー数集計（LEFT JOIN） |
...
| `group_members` | オンライン（API 経由） | 現時点は SELECT（JOIN）のみ利用。直接操作 API は未実装 |
