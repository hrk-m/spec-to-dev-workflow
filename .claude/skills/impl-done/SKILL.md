...
また、実装過程で不要になった未使用のコード・ファイル（どこからも参照されない関数・型定義等）があれば削除してください。
UI コンポーネントそのものの削除は /impl の責務のため、ここでは行わないでください。
```
...
```
/steering を実行して docs/steering/ を現在の実装に同期してください。
また、実装過程で不要になった未使用のコード・ファイル（どこからも参照されない関数・CSS スタイル・型定義等）があれば削除してください。
UI コンポーネントそのものの削除は /impl の責務のため、ここでは行わないでください。
```
