...
> **現在のコードベースは判定基準に含めない。**
> 参照スキル（go-clean-arch など）に「既存コードを優先する」「近傍コードのパターンを優先する」といった記述があっても、それは実装エージェント向けの指示であり、plan-checker のチェックには適用しない。
> スキル定義と現コードが食い違う場合も `fail` として扱い、ユーザー確認を必須とする。
...
- `pass`: スキルと整合している
- `fail`: スキル定義に違反している。現コードと差異がある場合も含む。`fail 時の対応` フローを踏む
