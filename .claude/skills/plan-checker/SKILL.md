...
- `pass`: スキルと整合している
- `fail`: スキル違反。差異を `AskUserQuestion` で提示し、ユーザー確認後に PRD を修正して再チェックする
...
`fail` を見つけたら、まず `AskUserQuestion` で以下を確認する:

- どの差異があり、何を根拠に `fail` と判定したか
- PRD のどのセクションをどう直す想定か
- 要件は変えず実装方針だけを直すのか、要件自体の見直しが必要か

`AskUserQuestion` の回答で修正方針が確定したら、次の順番で PRD を修正する:
...
要件自体を変えないと整合しない場合は、その場で修正しない。差分を整理し、`AskUserQuestion` でユーザー合意を取ってから更新する。

## ユーザー確認フロー

`fail` が 1 件でもある場合、次の流れを必ず踏む:

1. `AskUserQuestion` で差異を提示する
2. 選択肢では「提案どおり修正する」「要件を見直す」「今回は修正しない」を最低限提示する
3. ユーザーが "Other" で補足した場合は、その内容を優先して反映する
4. 回答内容を PRD に反映する
5. 再度 `plan-checker` を実行する

ユーザー確認なしに PRD を自動で確定させない。
