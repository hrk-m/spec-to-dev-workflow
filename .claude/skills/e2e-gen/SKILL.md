...
No.1〜No.10 の各カテゴリについて、同期後の PRD・`plans/shared/*.md` を根拠に「今回の変更に関連する観点が 1 つも導出できない」と判断したカテゴリは自動スキップする。
スキップしたカテゴリとその理由は確認開始前にまとめて提示する。

**確認手順（関連ありと判断したカテゴリのみ）**

1. カテゴリ名・説明と、今回の仕様に即した観点候補をナンバリングしてチャットに列挙する
   - 候補は固定文言でなく、今回の要件・仕様に言い換えた具体的な文言にする
2. `AskUserQuestion` で `OK / 修正 / コメント` を聞く
   - **OK** — 提示内容をそのまま採用してカテゴリを確定し、次へ進む
   - **修正** — ユーザーのコメントに従って観点を修正し、再提示して再確認する
   - **コメント** — コメントを反映して再提示して再確認する
3. カテゴリが確定したら次のカテゴリへ進む

関連ありと判断した全カテゴリの確認が終わるまで Step 4 に進まない。
確定した観点だけを Step 4 のテストケース候補に落とし込む。
