...
- 不一致あり（最低要件・処理フロー内で解消可能）→ AskUserQuestion でユーザーに差異を提示し、最低要件の確定に戻ってループする
...
- Step 1：plans の最終同期（/e2e-gen 自身が実行する）
- 機能固有 PRD がある場合は api-sync、DB 変更がある場合は schema-sync を実行し、対象 PRD と plans/schema.md を必要に応じて更新する
- つまり、plans の最終同期と必要に応じた PRD / schema 更新は /e2e-gen の責務とする
- 停止条件①（最低要件 / API 契約 / エラー挙動 / UX フローの解釈が揺れる差分）→ /plan に戻す
- 停止条件②（実装方針 / 追加ファイル / テスト方針の差分）→ 仕様・設計の問題なら /plan、実装のみなら /impl に戻す
- いずれにも該当しない場合は次のステップへ進む
- Step 2：既存状況の把握（並列）
...
- Step 3：10 カテゴリ観点の確認
- No.1 から No.10 まで、要件 / 仕様に照らしてカテゴリを 1 つずつ確認する
- 各カテゴリごとに、今回の要件 / 仕様に沿った具体観点を洗い出し、AskUserQuestion(multiSelect) で採否を確認する
- 各カテゴリの選択肢には「今回の変更では対象外」を含め、10 カテゴリすべての確認が終わってから次へ進む
- Step 4：テストケースリストの確定
...
- テスト失敗（/e2e-gen 内で解決できない実装のみの問題、または BE・FE 修正が必要）→ ループを停止し、/impl に戻す
- テスト失敗（仕様・設計の問題）→ ループを停止し、/plan に戻す
...
- E2E 失敗（仕様・設計の問題 / plans 同期結果が PRD・API 契約・UX フローに影響する差分）→ /plan に戻る
- E2E 失敗（/e2e-gen 内で解決できない実装のみの問題、または BE・FE 修正が必要）→ /impl に戻る
- /e2e-gen から戻った場合: /plan に戻った場合は /plan から、/impl に戻った場合は /impl から、それぞれ /arch-refactor REVIEWED まで修正サイクルを完走してから再度 /e2e-gen に入る
