...
FSD の entities レイヤー。ドメインエンティティの型定義と Pure モデルを配置する。ビジネスロジックは持たない。

- `user/` — ユーザーエンティティ（`model/user.ts` に `User` 型。`first_name`・`last_name`・`id`
  フィールドを持つ）
- 各 entity は `index.ts`（barrel export）を Public API として公開する

> **注意**: グループ詳細画面で使用する `UserSummary` 型は `pages/group-detail/model/group-detail.ts`
> に定義されており、`entities/user` の `User` 型とは別物。`UserSummary` はページ固有の型として
> `pages/` に留まる。`entities/user` の `User`
> はユーザー一覧など複数画面で再利用する型を想定している
