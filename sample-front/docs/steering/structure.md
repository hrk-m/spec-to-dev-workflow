...
3. **IntersectionObserver による自動追加取得**:
   センチネル要素（`<div ref={sentinelRef} />`）が viewport に入ったとき、`lastBatchSize === FETCH_LIMIT`（前回取得が上限件数）であれば次の 100 件を追加取得する。`lastBatchSize`
